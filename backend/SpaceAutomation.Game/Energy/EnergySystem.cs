...
public sealed class EnergySystem(World world)
{
    ...
    {
        var nodes = Nodes;
        if (!nodes.TryGetValue(firstId, out var first) || !nodes.TryGetValue(otherId, out var other)){
            return CommandResult.Reject("invalid_energy_endpoint");
        }

        if (firstId == otherId){
            return CommandResult.Reject("self_connection");
        }

        if (first is EnergyEquipment { InstalledIn: not null } || other is EnergyEquipment { InstalledIn: not null }){
            return CommandResult.Reject("connect_through_host");
        }

        if (first is IMobile { HasPendingMovement: true } || other is IMobile { HasPendingMovement: true }){
            return CommandResult.Reject("movement_pending");
        }

        if (!first.Energy.Connections.Contains(otherId)) {
            first.Energy.Connections.Add(otherId);
            other.Energy.Connections.Add(firstId);
        }
    ...
    public CommandResult Disconnect(string firstId, string otherId)
    {
        ...
        if (!first.Energy.Connections.Remove(otherId)){
            return CommandResult.Reject("not_connected");
        }
    ...
    private IEnumerable<IEnergyNode[]> Groups()
    {
        ...
        foreach (var obj in nodes.Values){
            if (obj is EnergyEquipment { InstalledIn: { } host }) {
                edges[obj.Id].Add(host);
                edges[host].Add(obj.Id);
            }
        }
        ...
        foreach (var group in Groups())
        {
            ...
            foreach (var obj in group.OfType<IConsumer>())
            {
                ...
                if (obj.Consumer.Demand <= available){
                    obj.Consumer.Received = obj.Consumer.Demand;
                    used += obj.Consumer.Demand;
                    available -= obj.Consumer.Demand;
                }
            }
            ...
            foreach (var obj in batteries) {
                var drawn = Math.Min(obj.Storage.Charge, deficit);
                obj.Storage.Charge -= drawn; deficit -= drawn;
            }

            var surplus = Math.Max(0, generated - used);

            foreach (var obj in batteries) {
                var stored = Math.Min(obj.Storage.Capacity - obj.Storage.Charge, surplus);
                obj.Storage.Charge += stored;
                surplus -= stored;
            }
        }
    }
