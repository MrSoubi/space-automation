...
public class Rover : GameObject, IEnergyNode, IMobile, IComponentHost
{
    ...
    [Observed("last_move_result")] public string? LastMoveResult { get; private set; }
    [Saved("_movement")] public Vector2? PendingMovement { get; private set; }
    ...
    public CommandResult Move(Vector2 direction,
                              double speed)
    {
        if (!direction.IsFinite || direction == Vector2.Zero) return CommandResult.Reject("invalid_direction");
        if (!Rules.Nonnegative(speed)) return CommandResult.Reject("invalid_speed");
        if (PendingMovement is not null) return CommandResult.Reject("movement_already_requested");
        if (Energy.Connections.Count > 0) return CommandResult.Reject("connected_to_grid");
        if (Motor is not { } motor) return CommandResult.Reject("missing_motor");
    ...
    public CommandResult ReplaceComponent(string slot,
                                          string? component_id)
    {
        if (slot is not ("battery" or "motor" or "solar_panel")) return CommandResult.Reject("invalid_slot");
        if (PendingMovement is not null) return CommandResult.Reject("movement_pending");
        if (Energy.Connections.Count > 0) return CommandResult.Reject("connected_to_grid");
        ...
        if (component_id is not null)
        {
            ...
            if (!compatible) return CommandResult.Reject("incompatible_component");
            if (component_id == oldId) return CommandResult.Ok();
            if (replacement!.InstalledIn is not null) return CommandResult.Reject("component_installed");
            if (replacement.Energy.Connections.Count > 0) return CommandResult.Reject("component_connected");
            if (replacement.Position != Position) return CommandResult.Reject("component_out_of_reach");
        }
        if (World.Find<EnergyEquipment>(oldId) is { } old) { old.InstalledIn = null; old.Position = Position; }
        switch (slot) { case "battery": Energy.StorageId = component_id; break; case "motor": Energy.ConsumerId = component_id; break; default: Energy.SourceId = component_id; break; }
        if (replacement is not null) replacement.InstalledIn = Id;
        return CommandResult.Ok();
    }
    ...
    public override void Update()
    {
        if (PendingMovement is { } movement)
        {
            ...
            { Position += movement; LastMoveResult = "moved"; }
            else LastMoveResult = "not_enough_energy";
            PendingMovement = null;
        }
        if (Motor is { } drive) drive.Consumer.Demand = 0;
        foreach (var id in new[] { Energy.StorageId, Energy.SourceId, Energy.ConsumerId })
            if (World.Find<EnergyEquipment>(id) is { } part) part.Position = Position;
    }
