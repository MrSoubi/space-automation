...
public sealed class World
{
    ...
    {
        if (_objects.ContainsKey(obj.Id)) throw new InvalidOperationException($"Duplicate object id: {obj.Id}");
        if (obj is EnergyEquipment equipment) equipment.InitializePort();
        obj.ValidateState(); obj.Attach(this); _objects.Add(obj.Id, obj);
    }
    public T? Find<T>(string? id) where T : GameObject => id is not null && _objects.TryGetValue(id, out var obj) ? obj as T : null;
    public void Validate() { foreach (var obj in _objects.Values) obj.ValidateState(); Energy.Validate(); }
    public void Advance()
    {
        var ordered = _objects.Values.OrderBy(x => x.Id, StringComparer.Ordinal).ToArray();
        foreach (var obj in ordered) obj.PrepareTick();
        Energy.Resolve();
        foreach (var obj in ordered) obj.Update();
        Tick++;
    }
}
